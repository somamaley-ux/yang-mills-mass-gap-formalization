...
theorem YangMillsNativeCrossHeartFactorizationTriangleDataExportStatement
  ...
  (let T :=
   ...
   T.stepwise_endpoint_image = T.direct_constructive_endpoint_image) :=
  YangMillsNativeCrossHeartFactorizationTriangleDataWitnessStatement
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
