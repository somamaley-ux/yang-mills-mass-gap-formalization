...
theorem YangMillsNativeConstructiveEndpointFactorizationLawExportStatement
  ...
  Nonempty (YMNativeConstructiveEndpointFactorizationLaw RC RD RE) /\
  ...
  (let L :=
   ...
   L.stepwise_endpoint_image = L.direct_endpoint_image) :=
  YangMillsNativeConstructiveEndpointFactorizationLawWitnessStatement
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
