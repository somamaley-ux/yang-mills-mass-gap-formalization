...
theorem YangMillsNativeCrossHeartFactorizationLawTriangleCoherenceExportStatement
  ...
  (let C :=
   ...
   C.law_assembly.vacuum_gap_endpoint.direct_endpoint_image =
     C.law_assembly.constructive_endpoint.direct_endpoint_image) :=
  YangMillsNativeCrossHeartFactorizationLawTriangleCoherenceWitnessStatement
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
