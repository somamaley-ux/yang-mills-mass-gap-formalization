...
/--
Minimal derived witness layer for the unitary-mixing paper.

`C` is the bookkeeping type and `W` is a canonical witness space. This layer
records the stronger derivation shape where operator-structured and residual
mixing bookkeeping are both read off the same witness channel.
-/
structure UnitaryMixingWitnessData (C W : Type) where
  witness : C → W
  operatorWitness : C → W
  residualWitness : C → W
...
-/
theorem operator_and_residual_mixing_share_witness
  {C W : Type}
...
-/
theorem no_unitary_mixing_witness_split
  {C W : Type}
  (D : UnitaryMixingWitnessData C W)
  (h_operator :
    ∀ c : C, D.operatorWitness c = D.witness c)
  ...
  exact hNe (operator_and_residual_mixing_share_witness D h_operator h_residual c)
