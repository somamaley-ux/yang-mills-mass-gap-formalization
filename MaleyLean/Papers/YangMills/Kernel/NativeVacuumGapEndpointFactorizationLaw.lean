...
/--
Typed law object exposing the vacuum-gap-to-endpoint factorization equality as
its own named native law package.
-/
structure YMNativeVacuumGapEndpointFactorizationLaw
  ...
  stepwise_definition :
    stepwise_endpoint_image = factorization.composite_endpoint_image
  direct_definition :
    direct_endpoint_image = factorization.direct_endpoint_image
  endpoint_factorization_law :
    stepwise_endpoint_image = direct_endpoint_image

def YangMillsNativeVacuumGapEndpointFactorizationLawData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeVacuumGapEndpointFactorizationLaw RC RD RE := by
  let F :=
  ...
  refine
    { factorization := F
      stepwise_endpoint_image := F.composite_endpoint_image
      direct_endpoint_image := F.direct_endpoint_image
      stepwise_definition := rfl
      direct_definition := rfl
      endpoint_factorization_law := F.factorization_agrees }

theorem YangMillsNativeVacuumGapEndpointFactorizationLawWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (let L :=
    YangMillsNativeVacuumGapEndpointFactorizationLawData
      ...
      hww vtm vobs vrsec vcorr vgapf vhgap
      hE hP evac etest efield
   L.stepwise_endpoint_image = L.direct_endpoint_image) := by
  let L :=
    YangMillsNativeVacuumGapEndpointFactorizationLawData
      RC RD RE
      htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
  ...
  exact
    And.intro ⟨L⟩ <|
      And.intro ⟨L.factorization⟩ <|
        And.intro L.direct_endpoint_image L.endpoint_factorization_law
