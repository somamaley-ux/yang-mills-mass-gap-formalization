...
/--
Typed assembly collecting the three native-style semantic law packages into one
higher-level Yang-Mills semantic-law object.
-/
structure YMNativeLawAssembly
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  constructive : YMConstructiveExtendAssembleLawPackage RC
  vacuum_gap : YMVacuumGapTransportRealizeLawPackage RD
  endpoint : YMEndpointCorrelationLawPackage RE

def YangMillsNativeLawAssemblyData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
...
theorem YangMillsNativeLawAssemblyExistsStatement
  ...
  (etest : (YangMillsEndpointSemanticBundleData RE hE hP).test_function_space)
  (efield : (YangMillsEndpointSemanticBundleData RE hE hP).field_family) :
  Nonempty (YMNativeLawAssembly RC RD RE) := by
  exact ⟨YangMillsNativeLawAssemblyData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield⟩

theorem YangMillsNativeLawAssemblyWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  have hA := YangMillsNativeLawAssemblyExistsStatement
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
  have hC := YangMillsConstructiveExtendAssembleLawPackageExistsStatement
    RC htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
  have hV := YangMillsVacuumGapTransportRealizeLawPackageExistsStatement
    RD hww vtm vobs vrsec vcorr vgapf vhgap
  have hE' := YangMillsEndpointCorrelationLawPackageExistsStatement
    RE hE hP evac etest efield
  exact And.intro hA <| And.intro hC <| And.intro hV hE'
