...
/--
Canonical route-level closure object for the Yang--Mills spine.

This stays entirely on the public-scope plus route-output side, so it avoids the
arbitrary theorem-part landing maps used by the older theorem-facing assembly.
-/
structure YMCanonicalRouteClosure
...
def YangMillsCanonicalRouteClosureData
  ...
  let W :=
    YangMillsCanonicalTheoremAssemblyWitnessData
      ...
      hAroute
  exact
    { public_scope_witness := W.public_scope_witness
      partC_route_witness := W.partC_route_witness
      partD_route_witness := W.partD_route_witness
      endpoint_route_witness := W.endpoint_route_witness }

theorem YangMillsCanonicalRouteClosureExistsStatement
  (S : YMLoadBearingSpine)
  (hp1 : S.packet1_uv_gate)
  (hp2 : S.packet2_entrance)
  (hCready1 : S.partC_route.flowed_state_ready)
  ...
  (hEready3 : S.endpoint_route.endpoint_packet_ready)
  (hAroute : ym_public_scope_route S) :
  Nonempty (YMCanonicalRouteClosure S) := by
  exact
...
theorem YangMillsCanonicalRouteClosureExportStatement
  (S : YMLoadBearingSpine)
  (hp1 : S.packet1_uv_gate)
  (hp2 : S.packet2_entrance)
  (hCready1 : S.partC_route.flowed_state_ready)
  ...
  let C :=
    YangMillsCanonicalRouteClosureData
      ...
      hAroute
  exact And.intro ⟨C⟩ <|
    And.intro C.public_scope_witness <|
      And.intro C.partC_route_witness <|
        And.intro C.partD_route_witness C.endpoint_route_witness
