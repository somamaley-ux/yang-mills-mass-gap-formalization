...
/--
Named higher-level consequences extracted from the unified native-law
assembly.
-/
structure YMNativeLawConsequences
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  constructive_bounded_base :
...
def YangMillsNativeLawConsequencesData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeLawConsequences RC RD RE := by
  ...
  refine
    { constructive_bounded_base := A.constructive.bounded_base_witness
      vacuum_gap_ready := A.vacuum_gap.minkowski_gap_witness
      endpoint_exact := A.endpoint.exact_endpoint_witness }

theorem YangMillsNativeLawConsequencesExistsStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (etest : (YangMillsEndpointSemanticBundleData RE hE hP).test_function_space)
  (efield : (YangMillsEndpointSemanticBundleData RE hE hP).field_family) :
  Nonempty (YMNativeLawConsequences RC RD RE) := by
  exact ⟨YangMillsNativeLawConsequencesData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield⟩

theorem YangMillsNativeLawConsequencesWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  let C := YangMillsNativeLawConsequencesData
    RC RD RE
    htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
    hww vtm vobs vrsec vcorr vgapf vhgap
    hE hP evac etest efield
  exact And.intro ⟨C⟩ <| And.intro C.constructive_bounded_base <| And.intro C.vacuum_gap_ready C.endpoint_exact
