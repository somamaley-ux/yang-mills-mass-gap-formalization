...
/--
Full triangle-coherence object for the native factorization-law layer. This
records the gap-side match, the induced endpoint match through the vacuum-gap
route, and the final agreement with the direct constructive endpoint law.
-/
structure YMNativeCrossHeartFactorizationLawTriangleCoherence
...
def YangMillsNativeCrossHeartFactorizationLawTriangleCoherenceData
  ...
  YMNativeCrossHeartFactorizationLawTriangleCoherence RC RD RE := by
  let A :=
  ...
  refine
    { law_assembly := A
      gap_law_agrees := rfl
      induced_endpoint_law_agrees := rfl
      direct_endpoint_law_agrees := rfl }

theorem YangMillsNativeCrossHeartFactorizationLawTriangleCoherenceWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (let C :=
   ...
   C.law_assembly.vacuum_gap_endpoint.direct_endpoint_image =
     C.law_assembly.constructive_endpoint.direct_endpoint_image) := by
  let C :=
    YangMillsNativeCrossHeartFactorizationLawTriangleCoherenceData
      RC RD RE
      htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
  ...
  exact
    And.intro ⟨C⟩ <|
      And.intro ⟨C.law_assembly⟩ C.direct_endpoint_law_agrees
