...
/--
Typed assembly collecting the three native cross-heart factorization law
objects into one higher-level package.
-/
structure YMNativeCrossHeartFactorizationLawAssembly
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  constructive_vacuum_gap :
    YMNativeConstructiveVacuumGapFactorizationLaw RC RD RE
  vacuum_gap_endpoint :
    YMNativeVacuumGapEndpointFactorizationLaw RC RD RE
  constructive_endpoint :
    YMNativeConstructiveEndpointFactorizationLaw RC RD RE

def YangMillsNativeCrossHeartFactorizationLawAssemblyData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeCrossHeartFactorizationLawAssembly RC RD RE := by
  refine
    { constructive_vacuum_gap :=
        YangMillsNativeConstructiveVacuumGapFactorizationLawData
          RC RD RE
          htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
          hww vtm vobs vrsec vcorr vgapf vhgap
          hE hP evac etest efield
      vacuum_gap_endpoint :=
        YangMillsNativeVacuumGapEndpointFactorizationLawData
          RC RD RE
          htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
          hww vtm vobs vrsec vcorr vgapf vhgap
          hE hP evac etest efield
      constructive_endpoint :=
        YangMillsNativeConstructiveEndpointFactorizationLawData
          RC RD RE
          htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
          hww vtm vobs vrsec vcorr vgapf vhgap
          hE hP evac etest efield }

theorem YangMillsNativeCrossHeartFactorizationLawAssemblyWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  Nonempty (YMNativeConstructiveVacuumGapFactorizationLaw RC RD RE) /\
  Nonempty (YMNativeVacuumGapEndpointFactorizationLaw RC RD RE) /\
  Nonempty (YMNativeConstructiveEndpointFactorizationLaw RC RD RE) := by
  let A :=
