...
/--
Typed coherence object for the named law pair on the native factorization-law
triangle. This records that the gap-side and endpoint-side named laws are built
over the same packaged triangle data.
-/
structure YMNativeCrossHeartFactorizationLawTriangleNamedLawsCoherence
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  named_laws : YMNativeCrossHeartFactorizationLawTriangleNamedLaws RC RD RE
  shared_triangle_package :
    named_laws.gap_law.triangle_package = named_laws.endpoint_law.triangle_package

def YangMillsNativeCrossHeartFactorizationLawTriangleNamedLawsCoherenceData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeCrossHeartFactorizationLawTriangleNamedLawsCoherence RC RD RE := by
  let L :=
  ...
  refine
    { named_laws := L
      shared_triangle_package := rfl }

theorem YangMillsNativeCrossHeartFactorizationLawTriangleNamedLawsCoherenceWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  (let C :=
    YangMillsNativeCrossHeartFactorizationLawTriangleNamedLawsCoherenceData
      ...
      hww vtm vobs vrsec vcorr vgapf vhgap
      hE hP evac etest efield
   C.named_laws.gap_law.triangle_package = C.named_laws.endpoint_law.triangle_package) := by
  let C :=
    YangMillsNativeCrossHeartFactorizationLawTriangleNamedLawsCoherenceData
      RC RD RE
      htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
  ...
  exact
    And.intro ⟨C⟩ <|
      And.intro ⟨C.named_laws⟩ C.shared_triangle_package
