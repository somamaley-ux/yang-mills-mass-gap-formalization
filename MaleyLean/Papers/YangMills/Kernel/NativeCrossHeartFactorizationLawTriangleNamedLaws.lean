...
/--
Typed package collecting the named gap-side and endpoint-side laws for the
native factorization-law triangle.
-/
structure YMNativeCrossHeartFactorizationLawTriangleNamedLaws
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  gap_law : YMNativeCrossHeartFactorizationLawTriangleGapLaw RC RD RE
  endpoint_law : YMNativeCrossHeartFactorizationLawTriangleEndpointLaw RC RD RE

def YangMillsNativeCrossHeartFactorizationLawTriangleNamedLawsData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeCrossHeartFactorizationLawTriangleNamedLaws RC RD RE := by
  refine
    { gap_law :=
        YangMillsNativeCrossHeartFactorizationLawTriangleGapLawData
          RC RD RE
...
theorem YangMillsNativeCrossHeartFactorizationLawTriangleNamedLawsWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  Nonempty (YMNativeCrossHeartFactorizationLawTriangleGapLaw RC RD RE) /\
  Nonempty (YMNativeCrossHeartFactorizationLawTriangleEndpointLaw RC RD RE) := by
  let L :=
  ...
  exact
    And.intro ⟨L⟩ <|
      And.intro ⟨L.gap_law⟩ ⟨L.endpoint_law⟩
