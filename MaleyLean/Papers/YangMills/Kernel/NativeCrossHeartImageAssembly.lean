...
/--
Typed assembly collecting the three native cross-heart image bridges into one
higher-level object.
-/
structure YMNativeCrossHeartImageAssembly
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  constructive_vacuum_gap : YMNativeConstructiveVacuumGapStateBridge RC RD RE
  vacuum_gap_endpoint : YMNativeVacuumGapEndpointImageBridge RC RD RE
  constructive_endpoint : YMNativeConstructiveEndpointImageBridge RC RD RE

def YangMillsNativeCrossHeartImageAssemblyData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeCrossHeartImageAssembly RC RD RE := by
  refine
    { constructive_vacuum_gap :=
      ...
      constructive_endpoint :=
        YangMillsNativeConstructiveEndpointImageBridgeData
          RC RD RE
          htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
          hww vtm vobs vrsec vcorr vgapf vhgap
          hE hP evac etest efield }

theorem YangMillsNativeCrossHeartImageAssemblyWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  Nonempty (YMNativeConstructiveEndpointImageBridge RC RD RE) := by
  let A :=
