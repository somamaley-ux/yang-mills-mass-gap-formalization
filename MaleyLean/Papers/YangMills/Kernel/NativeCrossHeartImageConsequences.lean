...
/--
Named higher-level consequences extracted from the native cross-heart image
assembly.
-/
structure YMNativeCrossHeartImageConsequences
    (RC : YMConstructiveRoute)
    (RD : YMVacuumGapRoute)
    (RE : YMEndpointCore) where
  constructive_state_image :
    PLift RD.reconstruction_package.minkowski_gap_ready
  vacuum_gap_endpoint_image :
    RE.endpoint_object.exact_local_net_endpoint
  constructive_endpoint_image :
    RE.endpoint_object.exact_local_net_endpoint

def YangMillsNativeCrossHeartImageConsequencesData
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  ...
  YMNativeCrossHeartImageConsequences RC RD RE := by
  let A :=
  ...
  refine
    { constructive_state_image := A.constructive_vacuum_gap.state_level_vacuum_gap_ready
      vacuum_gap_endpoint_image := A.vacuum_gap_endpoint.endpoint_image
      constructive_endpoint_image := A.constructive_endpoint.endpoint_image }

theorem YangMillsNativeCrossHeartImageConsequencesWitnessStatement
  (RC : YMConstructiveRoute)
  (RD : YMVacuumGapRoute)
  (RE : YMEndpointCore)
  (htrunc : RC.finite_truncation_ready)
  ...
  exact
    And.intro ⟨C⟩ <|
      And.intro ⟨A⟩ <|
        And.intro ⟨C.constructive_state_image⟩ <|
