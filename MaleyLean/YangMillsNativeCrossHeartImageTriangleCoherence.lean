...
theorem YangMillsNativeCrossHeartImageTriangleCoherenceExportStatement
  ...
  (let C :=
   ...
   C.image_assembly.vacuum_gap_endpoint.endpoint_image =
     C.image_assembly.constructive_endpoint.endpoint_image) := by
  exact
    YangMillsNativeCrossHeartImageTriangleCoherenceWitnessStatement
      RC RD RE
      htrunc hext hcompat hunion cwin cbridge cbase cstate cunion
      hww vtm vobs vrsec vcorr vgapf vhgap
      hE hP evac etest efield
