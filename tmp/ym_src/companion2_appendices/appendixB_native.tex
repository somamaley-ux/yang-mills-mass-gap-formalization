...
\item reflection positivity for the capwise positive-time algebra is read only through RP-safe bounded positive-time representatives in $A_+$, so the corresponding OS pairings are inherited from the tuned bounded Wilson base state up to the $o(1)$ replacement error already supplied by the finite-cap insertion package;
...
Fix the engineering cutoff $\Delta_{\max}$ and abbreviate the capwise sharp-local algebra by $A_{\mathrm{loc}}^{(\le \Delta_{\max})}$. The theorem-facing finite-cap bridge is now explicit: \Cref{thm:LaneA_closure_bridge} packages, from Source Corollary~F.330A, Source Theorem~F.331, and Source Corollary~5.70, the unique positive unital capwise functional on the abstract mixed algebra together with the RP-through-bounded-positive-time mechanism. Source Theorem~5.74 then adds only the identification of the formal centered symbols with the renormalized local generators, the capwise OS field realization, and the compatibility clauses later split off as Source Corollary~5.75.
...
\dependencyledger
  ...
  {none beyond the standard finite-cap OS reconstruction step already triggered by the reflection-positive capwise state furnished through \Cref{thm:LaneA_closure_bridge}}
