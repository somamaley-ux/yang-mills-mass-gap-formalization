...
\paragraph{Phase-VIII operator-domain attack checklist.}
A hostile operator-theoretic audit of Packet~8 should therefore test only four questions. First, do the dyadic OS matrix elements on $A_+$ exist exactly as stated in Theorem~\ref{thm:bounded-wilson-os-limit}? Second, does the Lane~A seam identify that bounded base state inside the sharp-local OS reconstruction before any all-$t$ statement is read? Third, does the passage from bounded observables to sharp-local vectors use only density of $A_+$ together with capwise finite-graph-norm approximants from the common invariant core? Fourth, does the final gap transfer use only semigroup contraction plus the boxed spectral input after those steps are complete? No other domain completion, non-dyadic lattice semigroup, or full-union graph-norm uniformity is available or needed on the live spine.
